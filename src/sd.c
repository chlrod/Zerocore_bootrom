...
int sd_copy(void *dst, uint32_t src_lba, uint32_t size)
{
    ...
    do
    {
        ...
        if (crc != crc_exp)
        {
            ...
            //         print_uart("\r\n");
            //     }
            //     print_uart_byte(tmp[i]);
            //     print_uart("    ");
            // }
            
            // print_uart("\r\nsize_index:");
            // print_uart_int((uint32_t)size_index);
            // print_uart("        crc:");
            // print_uart_int((uint32_t)crc);
            // print_uart("        crc_exp:");
            // print_uart_int((uint32_t)crc_exp);
            // print_uart("\r\n");
            __asm__ volatile(
                "nop;"
                "nop;"
            ...
            // print_uart("SD COPY ERROR CMD18 CRC\r\n");
            break;
        }

        if ((size_index % 4000) == 0 | size_index == size)
        {
            print_uart("\r\nsize_index:\r\n");
        }
